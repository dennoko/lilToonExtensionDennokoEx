...
#define BEFORE_MAIN \
    if (_CustomDecalEnabled > 0.5) { \
        ...
        float  _decalMask0 = LIL_SAMPLE_2D(_CustomDecalSharedMaskTex, sampler_linear_repeat, fd.uv0).r; \
        float3 _decalNTS = LIL_SAMPLE_2D(_CustomDecalNormalTex, sampler_linear_repeat, _decalUV0).rgb * 2.0 - 1.0; \
...
#define BEFORE_MAIN2ND \
    if (_CustomDecalEnabled > 0.5) { \
        ...
        float  _decalMask = LIL_SAMPLE_2D(_CustomDecalSharedMaskTex, sampler_linear_repeat, fd.uv0).r; \
        float4 _decalTex  = LIL_SAMPLE_2D(_CustomDecalTex, sampler_linear_repeat, _decalUV); \
...
#define BEFORE_REFLECTION \
    if (_CustomRefl2ndEnabled > 0.5) { \
        ...
        float4 _r2Tex = LIL_SAMPLE_2D(_CustomRefl2ndTex, sampler_linear_repeat, _r2UV); \
...
#define BEFORE_OUTPUT \
    if (_CustomDecalEnabled > 0.5) { \
        float  _dcMask = LIL_SAMPLE_2D(_CustomDecalSharedMaskTex, sampler_linear_repeat, fd.uv0).r; \
        float3 _dcNVS  = mul((float3x3)UNITY_MATRIX_V, fd.N); \
        float2 _dcMCUV = _dcNVS.xy * 0.5 + 0.5; \
        float4 _dcMCTex = LIL_SAMPLE_2D(_CustomDecalMatcapTex, sampler_linear_repeat, _dcMCUV); \
