...
> **2026-09-13 追記:** 本資料のマスク生成方式（NDMFビルド時ベイク＋メモリ上のエディタプレビュー）は、永続PNGアセット方式に置き換えた。`DennokoExMaskSync` と NDMF プラグインは削除済み。現行実装は [mask_packing_alternatives.md §11](mask_packing_alternatives.md) を参照。64上限の原因分析とRGBAパッキング自体の判断は引き続き有効。
...
5. 割当だけを自己修復の直後に連続してクリアする。変更通知や Undo があっても4回目で60秒停止し、その後に復旧すること。復旧キュー投入後、実際のベイクまでインポートやキュー待ちで1秒以上空く場合も同様に停止すること（復旧と正常判定が同じ update になっても回数を消さない）。通常の復旧（承認の次フレームでベイク）では直後の自己修復で回数がリセットされ、約2秒周期など毎周ではないクリアを「ループ」と誤判定して停止しないこと。
