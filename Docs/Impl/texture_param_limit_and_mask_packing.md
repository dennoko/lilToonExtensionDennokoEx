...
- ベイクが `null` を返すか例外になった場合は同期済みにせず、5秒間隔で再試行する。既存のプレビューは代替ベイクが成功するまで保持する。
- 自己修復ではテクスチャ自体の破棄も検出する。初回ベイクのキューが残っていても自己修復を実行し、変更通知経由の再評価で連続クリアのループ防止を迂回しない。
...
3. パッカー用シェーダーが利用できない状態で初回ベイクを試み、シェーダーを復旧する。ヒエラルキー操作なしで再試行されること。
