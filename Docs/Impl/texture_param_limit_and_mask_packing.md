...
5. 割当だけを自己修復の直後に連続してクリアする。変更通知や Undo があっても4回目で60秒停止し、その後に復旧すること。復旧キュー投入後、実際のベイクまでインポートやキュー待ちで1秒以上空く場合も同様に停止すること（復旧と正常判定が同じ update になっても回数を消さない）。復旧した割当が1秒以上維持された後の正常判定では回数がリセットされ、次のクリアを1回目として扱うこと。
